...
function decompose(n) {
    ...
    var denominator = 0
    if(n.valueOf() > 0) {
        leftDecimal = n.split('.')[0]
        rightDecimal = n.split('.')[1]
        numerator = leftDecimal + rightDecimal
        denominator = Math.pow(10, rightDecimal.length)
    ...
    do {
        ...
        if(temp == 1) {
            ...
            ans.push((1 + '/' + temp).toString())
        }
        numerator = (temp * numerator) - denominator
        denominator = denominator * temp
    } while (isFinite(denominator) == true)
    if(i > 0) {
        ans.unshift(i.toString())
    }
...
function decompose(n) {
    ...
    if (b < a) { res.push(String(a / b >> 0)); a %= b;}
    while (0 < a) { let dv = Math.ceil(b / a); res.push('1/' + dv); [a, b] = [a * dv - b, b * dv];}
    return res;
}